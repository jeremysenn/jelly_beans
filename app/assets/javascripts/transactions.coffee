...
jQuery ->
  $(document).on "turbolinks:load", ->
    ...
    if !$('#transaction_customer_id').hasClass('select2-hidden-accessible') # Dont' invoke select2 if already applied
    ...
    if !$('#commodity_item_commodity_id').hasClass('select2-hidden-accessible') # Dont' invoke select2 if already applied
